...
class ExamsStackedHorizontalBarChart extends StatelessWidget {
  ...
  ExamsStackedHorizontalBarChart(this.seriesList, this.isExam);

  static Widget fromModel(List<ExamSeparated> exams, int mode) {
    final exam = exams.first.recordType == 'Exam';
    return FutureBuilder(
      future: Future.microtask(() => _createData(exams, mode, isExam: exam)),
      builder: (context, AsyncSnapshot<List<charts.Series>> snapshot) {
        if (!snapshot.hasData) {
          return PlatformProgressIndicator();
        }
        return ExamsStackedHorizontalBarChart(snapshot.data, exam);
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      constraints: BoxConstraints.expand(height: isExam ? 140.0 : 47),
   ...
   */
  List<int> _generateExamsMeasureAxis(
          {@required int start, @required int num, @required int step}) =>
  ...
  static List<charts.Series<ExamResultPercent, String>> _createData(
    ...
    for (int i = 1; i < 5; i++) {
      percentages.add([
        ExamResultPercent(
          gender: 'M',
          percent: (_numberForMode(examsMale.where((e) => e.achievementLevel == i).toList(), mode) *
                      100 /
                      maleTotal)
                  .round() *
              (i < 3 ? -1 : 1),
        ...
        ExamResultPercent(
          gender: 'All',
          percent: (_numberForMode(exams.where((e) => e.achievementLevel == i).toList(), mode) *
                      100 /
                      allTotal)
                  .round() *
              (i < 3 ? -1 : 1),
        ...
        ExamResultPercent(
          gender: 'F',
          percent:
              (_numberForMode(examsFemale.where((e) => e.achievementLevel == i).toList(), mode) *
                          100 /
                          femaleTotal)
                      .round() *
                  (i < 3 ? -1 : 1),
    ...
    final percentFiller2 = [
      ExamResultPercent(
        gender: 'M',
        percent: 100 - percentages[2][0].percent - percentages[3][0].percent,
      ),
      ExamResultPercent(
        gender: 'All',
        percent: 100 - percentages[2][1].percent - percentages[3][1].percent,
      ),
      ExamResultPercent(
        gender: 'F',
        percent: 100 - percentages[2][2].percent - percentages[3][2].percent,
      ),
    ];
    ...
    return [
      charts.Series<ExamResultPercent, String>(
        ...
        data: percentages[1].where((element) => isExam || element.gender == 'All').toList(),
      ),
      charts.Series<ExamResultPercent, String>(
        id: 'Result1',
        ...
        domainFn: (ExamResultPercent sales, _) => sales.gender,
        measureFn: (ExamResultPercent sales, _) => sales.percent,
        data: percentages[0].where((element) => isExam || element.gender == 'All').toList(),
      ),
      charts.Series<ExamResultPercent, String>(
        id: 'ResultFiller1',
        ...
        domainFn: (ExamResultPercent sales, _) => sales.gender,
        measureFn: (ExamResultPercent sales, _) => sales.percent,
        data: percentFiller1.where((element) => isExam || element.gender == 'All').toList(),
      ),
      charts.Series<ExamResultPercent, String>(
        id: 'Result3',
        ...
        domainFn: (ExamResultPercent sales, _) => sales.gender,
        measureFn: (ExamResultPercent sales, _) => sales.percent,
        data: percentages[2].where((element) => isExam || element.gender == 'All').toList(),
      ),
      charts.Series<ExamResultPercent, String>(
        id: 'Result4',
        ...
        domainFn: (ExamResultPercent sales, _) => sales.gender,
        measureFn: (ExamResultPercent sales, _) => sales.percent,
        data: percentages[3].where((element) => isExam || element.gender == 'All').toList(),
      ),
      charts.Series<ExamResultPercent, String>(
        id: 'ResultFiller2',
        colorFn: (_, __) => const charts.Color(r: 245, g: 246, b: 248),
        domainFn: (ExamResultPercent sales, _) => sales.gender,
        measureFn: (ExamResultPercent sales, _) => sales.percent,
        data: percentFiller2.where((element) => isExam || element.gender == 'All').toList(),
      ),
    ];
  }
}
