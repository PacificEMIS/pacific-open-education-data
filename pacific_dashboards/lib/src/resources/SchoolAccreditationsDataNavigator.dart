...
class SchoolAccreditationsDataNavigator {
  ...
  List<SchoolAccreditationModel> _exams;
  SchoolAccreditationsModel _SchoolAccreditationsModel;

  SchoolAccreditationsDataNavigator(List<SchoolAccreditationModel> exams, SchoolAccreditationsModel SchoolAccreditationsModel) {
    _exams = exams;
    _examPages = _getExamPageNames();
    _SchoolAccreditationsModel = SchoolAccreditationsModel;
    _changeExamPage();
  }
  ...
  List<SchoolAccreditationModel> _getExamPage(String examPageName) {
  ...
  Map<String, List<SchoolAccreditationModel>> _getGroupedResults() {
  ...
  Map<String, Map<String, SchoolAccreditationModel>> _getExamResultsByBenchmark() {
    var groupedData = _getGroupedResults();
    Map<String, Map<String, SchoolAccreditationModel>> results =
        new Map<String, Map<String, SchoolAccreditationModel>>();
    groupedData.forEach((k, v) {
      var groupedByBenchmarkData = new Map<String, SchoolAccreditationModel>();
      for (var item in v) {
        if (groupedByBenchmarkData.containsKey(kNoTitleKey)) {
          groupedByBenchmarkData[kNoTitleKey] =
              SchoolAccreditationModel.sum(groupedByBenchmarkData[kNoTitleKey], item);
        } else {
  ...
  Map<String, Map<String, SchoolAccreditationModel>> _getExamResultsByYear() {
    var groupedData = _getGroupedResults();
    Map<String, Map<String, SchoolAccreditationModel>> results =
        new Map<String, Map<String, SchoolAccreditationModel>>();
    groupedData.forEach((k, v) {
      var groupedByYearData = new Map<String, SchoolAccreditationModel>();
      for (var item in v) {
        if (groupedByYearData.containsKey(item.examYear)) {
          groupedByYearData[item.examYear.toString()] =
              SchoolAccreditationModel.sum(groupedByYearData[item.examYear.toString()], item);
        } else {
  ...
  Map<String, Map<String, SchoolAccreditationModel>> _getExamResultsByState() {
    var groupedData = _getGroupedResults();
    Map<String, Map<String, SchoolAccreditationModel>> results =
        new Map<String, Map<String, SchoolAccreditationModel>>();
    groupedData.forEach((k, v) {
      var groupedByStateData = new Map<String, SchoolAccreditationModel>();
      for (var item in v) {
        var fullName =
            _SchoolAccreditationsModel.lookupsModel.getFullState(item.districtCode.toString());
        if (groupedByStateData.containsKey(fullName)) {
          groupedByStateData[fullName] =
              SchoolAccreditationModel.sum(groupedByStateData[fullName], item);
        } else {
  ...
  Map<String, Map<String, SchoolAccreditationModel>> getExamResults() {
